...
public class FlickerInput : MonoBehaviour
{
    ...
    public void OnFlick(InputAction.CallbackContext context)
    {
        ...
        else if (_values.Count > 0 && Vector2.Distance(value, _values.Last()) > 0.3f && value.magnitude != 0f)
        {
            _values.Add(value);
        }
    ...
    void CreateMoves() {
        ...
        for (int i = 0; i < _magnitudes.Count; i++)
        {
            ...
            if (_magnitudes[i - 1] >= EdgeValue && magnitude >= EdgeValue && movement.type == Move.Type.FF)
            {
                ...
                if (Vector2.SignedAngle(_values[i], _values[i - 1]) < 0f) movement.dir = Move.Dir.CC;
        ...
        if (_moves.Count == 0 && movement != null && movement.type == Move.Type.S)
